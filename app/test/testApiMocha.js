...
describe('Login Api /users/login', function(){
   ...
   it('Login API', function(done){
     api.post('/users/login')
       .set('x-api-key', 'lnx1337')
       .send({email:"emailModificado1@mail.com",password:'password1'})
    ...
    it('Login API Login incorrect',function(done){
      api.post('/users/login')
        .set('x-api-key', 'lnx1337')
        .send({email:"error@mail.com",password:'incorrect'})
        .expect(200).expect('Content-Type', /json/)
        .end(function(err,res){


          if (res.body[0].should.have.property('errors', 'User and password are incorrect')) {
             done();
          }
        });
    });
  ...
  it('#User login password null', function(done) {
    api.post('/users/login')
        ...
        .expect(200).expect('Content-Type', /json/)
        .end(function(err,res){
          if ( res.body.errors[0].should.have.property('error', 'Error: email is required') 
  ...
  it('#User login SQLinjection',function(done){
    ...
    api.post('/users/login')
        ...
        .end(function(err,res){
         ...
         if(res.body.errors[0].should.have.property('error', 'Error: email is invalid') 
...
describe('#Api List /users/list',function(){
  ...
  it('List /users/list correct',function(done){
    api.get('/users/list')
      ...
      .expect(200).expect('Content-Type', /json/)
      .end(function(err,res){     
        if(res.body[0].should.have.property('nombre','nombretest')){
          done();
        }
      });
   });
...
describe('API /users/add ',function(){
  ...
  it('API /users/add sending the correct parameters',function(done){
    api.post('/users/add')
        .set('x-api-key', 'lnx1337')
        .send({ nombre:"nuevo" , apellido: "apellidot", email:"email1292@testing.com" ,password:"passwordTesting" })
        .expect(200).expect('Content-Type', /json/)
        .end(function(err,res){
  ...
  */
  ...
  it('API add user sending data from an existing user  ',function(done){
    api.post('/users/add')
        .set('x-api-key', 'lnx1337')
        .send({ nombre:"usernametesting" , apellido: "apellidotesting",email:"email4@testing.com" ,password:"passwordTesting" })
        .expect(200).expect('Content-Type', /json/)
        .end(function(err,res){
           if(res.body[0].should.have.property('errors','Error: The username already exists')){
             done();  
           } 
        });
  });
  ...
  it('API add user sending null parameters',function(done){  
      api.post('/users/add')
        .set('x-api-key', 'lnx1337')
        .send({ nombre:"" , apellido: "",email:"" ,password:"" })
        .expect(200).expect('Content-Type', /json/)
        .end(function(err,res){
           if( res.body.errors[0].should.have.property('error', 'Error: nombre is required') &&
  ...
  it('API add user send SQLinjection',function(done){
          api.post('/users/add')
          .set('x-api-key', 'lnx1337')
          .send({ nombre:"'--" , apellido: "' or '1", email:"//" ,password:"--<alert>" })
          .expect(200).expect('Content-Type', /json/)
          .end(function(err,res){
             if ( res.body.errors[0].should.have.property('error', 'Error: nombre contains invalid characters') &&
...
describe('API GET /user/:id user findById ',function(){
  ...
  it('API GET  /user/:id send correct id',function(done){
    ...
    .expect(200).expect('Content-Type', /json/)
    .end(function(err,res){
      if(res.body[0].should.have.property('response','OK') 
 ...
 it('API GET /users/id sending a user id that does not exist',function(done){
   ...
   api.get('/users/1337')
    ...
    .expect(200).expect('Content-Type', /json/)
    .end(function(err,res){
      if(res.body[0].should.have.property('errors','Error: User Not exist')){
           done();  
        } 
    });
  });
  ...
  it('API GET  /users/:id sending incorrect id',function(done){
    ...
    .end(function(err,res){
      if(res.body[0].should.have.property('errors','Error: Invalid Request')){
          done(); 
        } 
    });
  });
...
describe('API PUT /users/:id  update user correct',function(){
  ...
  it('API PUT update correct all data',function(done){
     api.put('/users/edit/66')
        .set('x-api-key', 'lnx1337')
        .send({nombre:"nombretest",email:'emailModificado1@mail.com'})
        .expect(200).expect('Content-Type', /json/)
        .end(function(err,res){
          if(res.body[0].should.have.property('response','OK')){
            done(); 
  ...
  it('API PUT /users/:id update incorrect sending a user id that does not exist',function(done){
     api.put('/users/edit/1337')
        .set('x-api-key', 'lnx1337')
        .send({nombre:"nombremodificado",email:"emailmodig@hotmail.com"})
        .expect(200).expect('Content-Type', /json/)
        .end(function(err,res){
          if(res.body[0].should.have.property('errors','Error: User Not exist')){
             done();  
...
describe('API DELETE /user/:id user delete ',function(){
  ...
  it('delete correct',function(done){
    ...
    .expect(200).expect('Content-Type', /json/)
    .end(function(err,res){
       if (res.body[0].should.have.property('response', 'OK') &&  res.body[0].should.have.property('data', 'Deleted')) {
              done();
        } 
    });
